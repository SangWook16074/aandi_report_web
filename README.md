...
멘토링과정에서는 개인사정 또는 변심으로 인한 이탈자가 발생하였고, 기초 CS 과정에서 기준을 충족하지 못한 회원은 Framwork 과정으로 넘어갈 수 없었습니다.
이에 따라 더 이상 공개된 환경에서 과제를 제공할 수 없었기에 인증인가 요구사항이 발생하여 인증인가 기능을 추가하게 되었습니다.
빠른 도입을 위해 백엔드 팀원이 사용한 경험이 있었던 JWT 방식으로 구현하기로 결정하였고, Flutter Web 환경에서 JWT기반 인증인가 구현을 계획했습니다.
...
사용자가 로그인을 시도하면 서버로부터 AccessToken을 발급받게 되고, 클라이언트는 Session Storage에 토큰을 저장하고, 이후 스토리지에서 토큰을 가져와 여러 API를 수행할 수 있도록 구성했습니다.
